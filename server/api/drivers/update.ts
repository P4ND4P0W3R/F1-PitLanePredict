...
export default defineEventHandler(async (event) => {
    ...
    const { oldDriverId, newDriverId, code, permanentNumber, givenName, familyName, dateOfBirth, nationality, url, driverImage, teamId, token } = body;
    console.log(token);
    ...
    if (token) {
        try {
            ...
            if (userRole === 'admin') {
                ...
                const updatedDriver = await db.drivers.update({
                    where: {
                ...
                console.log(`Driver with driverId ${oldDriverId} has been updated.`);
                console.log(updatedDriver); // The updated driver object
