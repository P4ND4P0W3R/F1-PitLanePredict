    ...
    if (token) {
        try {
            ...
            if (userRole === 'admin') {
                ...
                const deletedDriver = await db.drivers.delete({
                    where: {
                        driverId: driverId,
                    },
                });

                console.log(`Driver with driverId ${driverId} has been deleted.`);
                console.log(deletedDriver); // The deleted driver object, if you want to do something with it
