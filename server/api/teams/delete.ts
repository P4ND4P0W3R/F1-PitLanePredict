    ...
    if (token) {
        try {
            ...
            if (userRole === 'admin') {
                ...
                console.log(`Driver with driverId ${teamId} has been deleted.`);
                console.log(deletedTeam); // The deleted driver object, if you want to do something with it

                return {
                    status: 200,
                    body: JSON.stringify(deletedTeam),
                };
            } else {
                // Deny the operation
